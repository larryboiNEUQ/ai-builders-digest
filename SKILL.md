...
1. **正文内联（主路径）**：每条头条、每个精选小节、每篇博客/播客，在该段**首段或标题下第一行**放可点链接：
   - 写法：`🔗 [原帖 @handle](url)` 或 `🔗 [Making-of](url) · [Artifacts 博文](url)`
   - 多条相关帖用 ` · ` 分隔，**最多 3 个**，其余放文末
2. **文末「延伸阅读 / 原帖」**：仍保留完整列表（便于收藏/复制），与正文链接**同一批 URL**，不要文末有、正文没有
3. **不要**只做「跳到文末锚点」而不给直达 x.com/博客的链接——读者要的是立刻打开原帖，不是在文档里多点一次
4. 表格里可加一列「原帖」或在要点末尾写 `([原帖](url))`
5. 飞书 Markdown 用标准 `[文字](https://...)`；创建/overwrite 后抽查链接可点
---
...
- [ ] **原帖链接（硬性）**：每条头条/精选/博客在**该段正文内**有可点 🔗，不只堆在文末
- [ ] 已在我的文档库，wiki 链接已发用户
