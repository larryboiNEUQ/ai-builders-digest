...
> 读者扫完头条后，只看这一节也要能「复述今天在吵什么 + 点开证据」。

1. **每条判断必须挂帖**：段末用 `>> [@handle · 主题](url)` 或文中嵌入链接；**禁止**只有口号没有来源。一条判断可挂 1–3 个相关帖（主证据优先）。
2. **不要太短、要讲人话**：
   - **禁止**电报体/标语体：如「双栈默认。」「配额是产品。」「Jevons 外溢。」
   - **要求**像跟同事复盘：先说**现象**，再说**这意味着什么/你可以怎么用**，必要时补一句**边界/别过度解读**。
   - 每条大约 **40–120 字**（2–4 句），不要写成小作文，也不要一句口号交差。
3. **判断 ≠ 复述头条标题**：要从「发生了什么」推进到「所以呢」（工作流、采购、选型、组织、风险）。
4. **飞书排版（避免格式乱）**：
   - **优先整篇 overwrite** 含判断节的干净 markdown；少用 `replace_range` 改「今日可带走的判断」（该模式对空行/多段落支持差，容易出现标题叠字、列表糊成一团）。
   - 每条判断写成**一条有序列表项内的连续段落**更稳：粗体结论 + 同一段里写解释 + 同一段末尾「来源：链接」，不要在一条里夹很多空行。
   - 链接文案用 `[@handle · 主题]`，不要一律「原帖」。
...
1. **额度已经是产品体验的一部分，不是售后补丁。** Sol 发布日连按两次 reset，评论区却仍在喊 5h 限流——说明发布周真正稀缺的是「跑得完 ambitious task 的配额」，不是权重本身。做 agent 产品的人，得把限流/reset 当成功能设计，而不是运维事故。来源：[@thsottiaux · 双 reset](https://x.com/...)、[@thsottiaux · full reset](https://x.com/...)

2. **……** ……人话展开…… 来源：[@handle · 主题](https://x.com/...)
```
---
...
- **情绪板**：用表格归纳 🎉😤🧩⚔️🛠，短。
- **判断 4–6 条**：见下方「今日可带走的判断」专规——**带帖、讲人话、别电报体**。
### 不要做（7-07/08 踩坑清单）
...
- [ ] **其他信号表**为 3 列（Builder / 要点 / **链接**），链接文案可辨识，无表外游离链接代替
- [ ] **每篇博客、每集播客**标题或段首有全文/收看链接（0 篇可写无内容）
- [ ] **今日可带走的判断**：4–6 条；每条 2–4 句人话 + 对应帖链接；无电报体口号
- [ ] 已在我的文档库，wiki 链接已发用户
